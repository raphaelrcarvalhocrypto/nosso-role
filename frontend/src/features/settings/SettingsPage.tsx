...
export default function Settings() {
  ...
  const syncPhotoPreview = async (photoRef: string) => {
    if (!photoRef) {
      setPhotoPreviewUrl('');
      return;
    }
    ...
    try {
      const resolvedUrl = await resolveCouplePhotoUrl(photoRef);
      setPhotoPreviewUrl(resolvedUrl);
    } catch (error) {
      console.error('Erro ao gerar preview da foto', error);
      setPhotoPreviewUrl(photoRef);
    }
  };
  ...
  useEffect(() => {
    const loadSettings = async () => {
      try {
        ...
        await syncPhotoPreview(appSettings?.couple_photo ?? '');
  ...
  const handlePhotoUpload = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const oldPath = extractCouplePhotoPath(settings.couple_photo);
      if (oldPath) {
        await supabase.storage.from(COUPLE_PHOTOS_BUCKET).remove([oldPath]);
      }

      setSettings({ ...settings, couple_photo: filePath });
      await syncPhotoPreview(filePath);
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) => {
                ...
                setSettings({ ...settings, couple_photo: value });
