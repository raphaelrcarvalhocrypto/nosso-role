...
const ROMANTIC_QUOTES = [
  "Você é a minha melhor aventura.",
  "O universo conspira a nosso favor.",
  "Sempre foi você, sempre será.",
...
export default function Dashboard() {
      ...
      try {
        ...
        if (profile?.couple_id) {
          ...
          const [{ count: datesCount, error: datesError }, { count: tripsCount, error: tripsError }, { count: wishesCount, error: wishesError }] =
            await Promise.all([
              supabase.from('dates').select('*', { count: 'exact', head: true }).eq('couple_id', profile.couple_id),
              supabase
                .from('trips')
  ...
  const updateTimers = (periods: Array<{start_date: string, end_date?: string}>, startDate?: string) => {
    ...
    if (!activePeriods || activePeriods.length === 0) {
      setDays(0); setHours(0); setMinutes(0); return;
    }
    ...
    >
      <motion.header variants={childVariants} className="mb-10 text-center md:text-left">
        <h2 className="text-4xl font-serif mb-2 text-slate-900 dark:text-white">
          Olá, as coisas estão <span className="text-rose-500 italic">maravilhosas!</span>
        </h2>
          ...
          <div className="flex-1 text-center md:text-left">
            ...
            <div className="flex flex-wrap justify-center md:justify-start gap-4">
              <div className="text-center">
                <motion.div key={days} initial={{ scale: 0.8 }} animate={{ scale: 1 }} className="text-5xl font-serif tabular-nums text-slate-800 dark:text-white">{days}</motion.div>
              ...
              <div className="text-center">
                <motion.div key={hours} initial={{ scale: 0.8 }} animate={{ scale: 1 }} className="text-5xl font-serif tabular-nums text-slate-800 dark:text-white">{hours}</motion.div>
                <div className="text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest mt-1">Horas</div>
              </div>
              <div className="text-5xl font-serif text-slate-300 dark:text-white/20">:</div>
              <div className="text-center">
                <motion.div key={minutes} initial={{ scale: 0.8 }} animate={{ scale: 1 }} className="text-5xl font-serif tabular-nums text-slate-800 dark:text-white">{minutes}</motion.div>
            ...
            {(!settings?.relationship_periods || settings.relationship_periods.length === 0) && !settings?.relationship_start_date && (
              <p className="text-sm text-rose-500 mt-4 bg-rose-50 dark:bg-rose-500/10 inline-block px-3 py-1.5 rounded-lg font-medium shadow-sm">Vão nas configurações e preencham a data de início!</p>
            )}
          </div>
        </div>
      </motion.div>
      ...
      <motion.div variants={containerVariants} className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <motion.div variants={childVariants} className="glass p-6 rounded-2xl border border-slate-200/80 dark:border-white/5 hover:border-slate-300 dark:hover:border-white/10 transition-colors shadow-md">
          <div className="flex items-start justify-between mb-4">
            ...
            <motion.span key={stats.dates} initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-3xl font-serif text-slate-800 dark:text-white">{stats.dates}</motion.span>
          </div>
          <p className="font-bold text-slate-800 dark:text-slate-200">Dates realizados</p>
          <p className="text-sm text-slate-500 mt-1">Momentos incríveis juntos.</p>
        </motion.div>
        ...
        <motion.div variants={childVariants} className="glass p-6 rounded-2xl border border-slate-200/80 dark:border-white/5 hover:border-slate-300 dark:hover:border-white/10 transition-colors shadow-md">
          <div className="flex items-start justify-between mb-4">
            ...
            <motion.span key={stats.trips} initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-3xl font-serif text-slate-800 dark:text-white">{stats.trips}</motion.span>
          </div>
          <p className="font-bold text-slate-800 dark:text-slate-200">Viagens pelo mundo</p>
          <p className="text-sm text-slate-500 mt-1">Nossa próxima parada nos espera.</p>
        </motion.div>
        ...
        <motion.div variants={childVariants} className="glass p-6 rounded-2xl border border-slate-200/80 dark:border-white/5 hover:border-slate-300 dark:hover:border-white/10 transition-colors shadow-md">
          <div className="flex items-start justify-between mb-4">
            ...
            <motion.span key={stats.wishes} initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-3xl font-serif text-slate-800 dark:text-white">{stats.wishes}</motion.span>
          </div>
          <p className="font-bold text-slate-800 dark:text-slate-200">Desejos a realizar</p>
          <p className="text-sm text-slate-500 mt-1">O futuro é promissor.</p>
        </motion.div>
      </motion.div>
    </motion.div>
  );
