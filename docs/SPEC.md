...
backend/
  firebase/                  # Legado da fase Firebase (nao utilizado no fluxo atual)
database/
...
Observacao: o app atual usa `firebase-applet-config.json` versionado. Para producao, avaliar migrar configuracoes publicas do Firebase para variaveis `NEXT_PUBLIC_FIREBASE_*` e manter segredos fora do repositorio.
